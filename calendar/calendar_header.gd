...
@export var calendar_occupancy_items_container: Container
...
func apply_day_numbers() -> void:
	#var counter: int = 0
	for counter in range(1, 32):
	#for calendar_header_field: CalendarHeaderField in calendar_header_fields_container.get_children():
		#counter += 1
		calendar_header_fields_container.get_child(counter - 1).label.text = "%02d" % counter
		calendar_occupancy_items_container.get_child(counter - 1).initialize(counter)


func update() -> void:
	update_labels()
	
	var first_weekday = Time.get_datetime_dict_from_datetime_string(Utils.get_date_string(calendar.selected_year, calendar.selected_month, 1), true).weekday
